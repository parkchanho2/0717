...
// 객체 직렬화를 위한 클래스 설계
// 1. 객체 직렬화 클래스를 설계하기 위해서 java.io 의 Externalizable 인터페이스를 구현 상속 받고 2개의 추상 메소드를 오버라이딩함.
...
public class Customer implements Externalizable{
	...
	public void writeExternal(ObjectOutput out) throws IOException {
		...
		// 문제) 각각 자바의 어떤 문법이 적용되었는지 기록하기.
		out.writeObject(new Integer(id)); // 1. 오토박싱(자바 기본타입이 래퍼 참조타입으로 자동형 변환) + 업캐스팅
		out.writeObject(name); // 2. 업캐스팅
		out.writeObject(new Integer(age)); // 3. 오토박싱 + 업캐스팅
		out.writeObject(new Double(height)); // 4. 오토박싱 + 업캐스팅
	}// 객체 단위로 기록
